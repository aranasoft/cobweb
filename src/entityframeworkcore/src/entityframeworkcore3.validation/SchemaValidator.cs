            ...
            foreach (var persistedColumn in persistedType.GetProperties()
                ...
                if (persistedColumn.GetDefaultValue() != null || persistedColumn.GetDefaultValueSql() != null || dbColumn.DefaultValueSql != null) {
                    if (persistedColumn.GetDefaultValue() != null && persistedColumn.GetDefaultValue()?.ToString().TrimParentheses() != dbColumn.DefaultValueSql.TrimParentheses()) {
                        valErrors.Add(
                            $"Column default value mismatch in {persistedType.GetTableName()} for column {persistedColumn.GetColumnName()}. Found: {dbColumn.DefaultValueSql.TrimParentheses() ?? "<none>"}, Expected: {persistedColumn.GetDefaultValue().ToString().TrimParentheses() ?? "<none>"}");
                    }
                    ...
                    if (persistedColumn.GetDefaultValueSql() != null && persistedColumn.GetDefaultValueSql().TrimParentheses() != dbColumn.DefaultValueSql.TrimParentheses()) {
                        valErrors.Add(
                            $"Column default value mismatch in {persistedType.GetTableName()} for column {persistedColumn.GetColumnName()}. Found: {dbColumn.DefaultValueSql.TrimParentheses() ?? "<none>"}, Expected: {persistedColumn.GetDefaultValueSql().TrimParentheses() ?? "<none>"}");
                    }

                    if (persistedColumn.GetDefaultValue() == null && persistedColumn.GetDefaultValueSql() == null && dbColumn.DefaultValueSql != null) {
                        valErrors.Add(
                            $"Column default value mismatch in {persistedType.GetTableName()} for column {persistedColumn.GetColumnName()}. Found: {dbColumn.DefaultValueSql.TrimParentheses() ?? "<none>"}, Expected: <none>");
                    }
                }
            }
